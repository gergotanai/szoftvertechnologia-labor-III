...
using MovieMinimalAPIJWT.Services;
...
(Movie movie, IMovieService service) => Create(movie, service))
    ...
    .Produces<Movie>(200, "application/json");
...
(IMovieService service) => List(service))
    .Produces<List<Movie>>(200, "application/json");
...
IResult Delete(int id, IMovieService service)
{
    ...
    if (!result) return Results.BadRequest("Movie not found");
    return Results.Ok(result);
}
